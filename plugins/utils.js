...
function initMiniByDumi() {
  try {
    ...
    // 1. 先删除dest旧文件夹再新建文件夹
    if (fs.existsSync(miniPagesDir)) fs.rmSync(miniPagesDir, { recursive: true })
    fs.mkdirsSync(miniPagesDir);

    // 2. 复制dumi下的demo
    const srcDemo = (item) => `${srcPath}/${item}/demo`;
    const temparr = fs.readdirSync(srcPath).filter(item => fs.existsSync(srcDemo(item)));
    ...
    fs.writeFileSync(`${destPath}/src/app.pages.json`, JSON.stringify(routes), 'utf8')
  } catch (error) {
